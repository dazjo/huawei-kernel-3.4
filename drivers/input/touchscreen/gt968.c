...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    u8  point_data[2 + 1 + 8 * GTP_MAX_TOUCH + 1]={GTP_READ_COOR_ADDR >> 8, GTP_READ_COOR_ADDR & 
    0xFF};
    u8  touch_num = 0;
    u8  finger = 0;
    ...
    GTP_DEBUG("pre_touch:%02x, finger:%02x.", pre_key, finger);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if (gpio_tlmm_config(GPIO_CFG(GTP_INT_PORT, 0,GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
                GPIO_CFG_ENABLE))
    {
        ret = -ENODEV;
    }
                
	if (gpio_tlmm_config(GPIO_CFG(GTP_RST_PORT, 0,GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
                GPIO_CFG_ENABLE))
    {
        ret = -ENODEV;
    }
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    {
		/* set touch probe flag. indicate the device is found */
